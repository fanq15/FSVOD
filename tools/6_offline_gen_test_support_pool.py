...
def main():
    ...
    support_dict['id'] = []
    support_dict['video_id'] = []
    support_dict['track_id'] = []
    support_dict['file_path'] = []

    #for dataType in ['train2017']: #, 'train2017']:
    set_crop_base_path = support_path #join(support_path, dataType)
    set_img_base_path = join(dataDir, 'images') #dataType)

    annFile = os.path.join(root_path, 'annotations/offline_support_fsvod_test.json') #'new_annotations/final_split_non_voc_instances_train2017.json')
    ...
    for img_id, id in enumerate(tao.get_img_ids()):

        img = tao.load_imgs([id])[0]
        '''
        if img_id % 100 == 0:
            print(img_id)
        img = tao.load_imgs([id])[0]
        if img['file_name'].startswith('GOT-10k'):
            if id % 10 != 0:
                continue
        elif img['file_name'].startswith('LaSOT'):
            if id % 3 != 0:
                continue
        else:
            pass
        '''
        anns = tao.load_anns(tao.get_ann_ids(img_ids=[id]))
        
        if len(anns) == 0:
            continue

        frame_crop_base_path = join(set_crop_base_path, os.path.join(*img['file_name'].split('/')[:-1]))
        ...
        for item_id, ann in enumerate(anns):
            ...
            if rect[0] < 0 or rect[1] < 0 or rect[2] <= 0 or rect[3] <=0:
                print(rect)
                continue
            file_path = join(frame_crop_base_path, img_name + '_{:04d}.jpg'.format(item_id))
            ...
            support_dict['video_id'].append(ann['video_id'])
            support_dict['track_id'].append(ann['track_id'])
            support_dict['file_path'].append(os.path.join(file_path))
...
if __name__ == '__main__':
    since = time.time()
    support_df = main()
    support_df.to_pickle("./offline_fsvod_test_support_df.pkl")
