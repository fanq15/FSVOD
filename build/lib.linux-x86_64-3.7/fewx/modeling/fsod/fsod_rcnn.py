...
class FsodRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        for i in range(B): # batch
            ...
            pos_end = pos_begin + self.support_shot #i * self.support_shot * self.support_way + self.support_shot
            ...
            neg_begin = pos_end #i * self.support_shot * self.support_way + self.support_shot
            neg_end = neg_begin + self.support_shot #i * self.support_shot * self.support_way + self.support_shot * 2
    ...
    def init_model(self):
        ...
        if not os.path.exists(offline_support_file_name):
            ...
            with open(offline_support_file_name, "rb") as hFile:
                self.offline_support_dict  = pickle.load(hFile, encoding="latin1")
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        # get query cls
        for item in batched_inputs[0]['annotations']:
            item_cls = item['category_id']
            cls_ls.append(item_cls) 

        online_cls = list(set(cls_ls))

        # get other cls
        support_way = 20 #5
        support_shot = 5
        if support_way > 1:
            all_ls = list(self.offline_support_dict['res4_avg'].keys())
            other_cls = list(set(all_ls) - set(cls_ls))
            cls_len = len(cls_ls)
        ...
        if len(online_cls) != 0:
            query_video = batched_inputs[0]['annotations'][0]['video_id']
            #print(query_video in self.offline_df['video_id'])
            if query_video not in self.offline_df['video_id']:
        ...
        for cls_id in online_cls:
            support_cls_df = self.online_df.loc[(self.online_df['category_id'] == cls_id) & (self.online_df['video_id'] != query_video), :].sample(support_shot, random_state=666).reset_index()
            ...
            res5_feature = self.roi_heads._shared_roi_transform([support_features[f] for f in self.in_features], support_box_all)
            support_box_features = res5_feature.mean(0, True)
            ########################
        ...
        for cls_id in offline_cls:
            res4_avg = self.offline_support_dict['res4_avg'][cls_id]
            ...
            # support branch ##################################
            support_box_features = self.offline_support_dict['res5_avg'][cls_id]
