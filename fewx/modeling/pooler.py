...
from fewx.layers import DefROIAlign 
...
class ROIPooler(nn.Module):
    ...
    def __init__(
        ...
        elif pooler_type == "DefROIAlign":
            self.level_poolers = nn.ModuleList(
                DefROIAlign(output_size, spatial_scale=scale, sampling_ratio=sampling_ratio, trans_std=0.1)
                for scale in scales
            )
        else:
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes], offsets=None):
        """
        Args:
        ...
        if num_level_assignments == 1:
            if self.pooler_type == "DefROIAlign":
                return self.level_poolers[0](x[0], pooler_fmt_boxes, offsets)
        ...
        for level, pooler in enumerate(self.level_poolers):
            ...
            if self.pooler_type == "DefROIAlign":
                output[inds] = pooler(x[level], pooler_fmt_boxes_level, offsets[inds])
            else:
